...
Revision ID: 72e409b0fcb7
Revises: 
Create Date: 2024-12-09 17:55:07.317522
...
def downgrade():
    ...
    with op.batch_alter_table('Booking', schema=None) as batch_op:
        batch_op.add_column(sa.Column('status', sa.VARCHAR(), server_default=sa.text("'pending'::character varying"), autoincrement=False, nullable=False))
        batch_op.add_column(sa.Column('created_at', postgresql.TIMESTAMP(timezone=True), server_default=sa.text('now()'), autoincrement=False, nullable=False))
        batch_op.alter_column('buyerID',
               existing_type=sa.Integer(),
               type_=sa.BIGINT(),
    ...
    sa.Column('BookingID', sa.INTEGER(), server_default=sa.text('nextval(\'"bookings_BookingID_seq"\'::regclass)'), autoincrement=True, nullable=False),
